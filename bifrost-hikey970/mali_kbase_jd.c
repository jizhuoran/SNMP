...
bool jd_submit_atom(struct kbase_context *kctx, const struct base_jd_atom_v2 *user_atom, struct kbase_jd_atom *katom)
{
	...
	sj_dbg_all("SJ: in jd_submit_atom with u1: %llu and u2: %llu and jc: %llu \n", user_atom->udata.blob[0], user_atom->udata.blob[1], user_atom->jc);
...
//SJ
	...
	for (i = 0; i < 2; i++) {
		...
		sj_dbg_all("SJ: dep_atom_number %d \n", dep_atom_number);
	...
	sj_dbg_all("SJ: jd_submit_atom  mess up with dependencies\n");
	...
	sj_dbg_all("SJ: jd_submit_atom will_fail\n");
...
//SJ
	...
	sj_dbg_all("SJ: jd_submit_atom sched_prio: %hhu \n", user_atom->prio);
	...
	if ((katom->core_req & BASE_JD_REQ_SPECIFIC_COHERENT_GROUP) &&
		...
		dev_warn(kctx->kbdev->dev,
				"Rejecting atom with invalid device_nr %d >= %d",
				katom->device_nr, kctx->kbdev->gpu_props.num_core_groups);
		katom->event_code = BASE_JD_EVENT_JOB_INVALID;
		ret = jd_done_nolock(katom, NULL);
		goto out;
	...
	sj_dbg_all("SJ: pass BASE_JD_REQ_EXTERNAL_RESOURCES\n");
	...
	if ((katom->core_req & BASE_JD_REQ_SOFT_JOB) == 0) {
		...
		sj_dbg_all("SJ: pass kbase_js_is_atom_valid\n");


	} else {

		sj_dbg_all("SJ: kbase_prepare_soft_job\n");
	...
	sj_dbg_all("SJ: pass IS_GPU_ATOM\n");
	...
	if ((katom->core_req & BASE_JD_REQ_SOFT_JOB_TYPE)
		...
		sj_dbg_all("SJ: is BASE_JD_REQ_SOFT_REPLAY\n");
		...
		sj_dbg_all("SJ: is BASE_JD_REQ_SOFT_JOB\n");
		...
		sj_dbg_all("SJ: is not a BASE_JD_REQ_DEP\n");
		...
		sj_dbg_all("SJ: is pure dependency\n");
		...
		bool uk6_atom)
{
	...
	sj_dbg_all("SJ: kbase_jd_submit %p %d \n", kctx, nr_atoms);
...
//SJ
	...
	for (i = 0; i < nr_atoms; i++) {
		...
		if (copy_from_user(&user_atom, user_addr,
			...
			sj_dbg_all("SJ: copy_from_user fail \n");


			break;
		}

		sj_dbg_all("SJ: we got the user data is %lu \n", user_atom.udata.blob[1]);
		
		if(user_atom.udata.blob[1] < 0x1000 && user_atom.udata.blob[1] != 0x0) {

			udata_sj = user_atom.udata.blob[1];

			user_atom.udata.blob[1] = 0x0;

			if (copy_to_user(user_addr, &user_atom,
						sizeof(user_atom)) != 0) {
				err = -EINVAL;
				sj_dbg_all("SJ: copy_to_user fail \n");

				break;
			}

			sj_dbg_all("SJ: going to mainuplite user_atom \n");

		}
		
		// user_atom.udata.blob[1] = udata_sj;	
...
#undef compiletime_assert_defined
		...
		while (katom->status != KBASE_JD_ATOM_STATE_UNUSED) {
			...
			sj_dbg_all("SJ: KBASE_JD_ATOM_STATE_UNUSED \n");
		...
		sj_dbg_all("SJ: Kneed_to_try_schedule_context %d \n", need_to_try_schedule_context);
		...
		sj_dbg_all("SJ: kbase_disjoint_event_potential \n");
...
int kbase_jd_submit_SJ_kernel(struct kbase_context *kctx,
		void __user *user_addr, u32 nr_atoms, u32 stride,
		bool uk6_atom)
{


	sj_dbg_all("SJ: in kbase_jd_submit_SJ_kernel %p %d \n", kctx, nr_atoms);
	...
	if (kbase_ctx_flag(kctx, KCTX_SUBMIT_DISABLED)) {
		sj_dbg_all("SJ: Attempt to submit to a context that has SUBMIT_DISABLED set on it \n");
		dev_err(kbdev->dev, "Attempt to submit to a context that has SUBMIT_DISABLED set on it");
		return -EINVAL;
	}

	if (stride != sizeof(base_jd_atom_v2)) {
		sj_dbg_all("SJ: Stride passed to job_submit doesn't match kernel \n");
		dev_err(kbdev->dev, "Stride passed to job_submit doesn't match kernel");
		return -EINVAL;
	}
	...
	sj_dbg_all("SJ: pass kbase_backend_get_current_flush_id \n");
	...
	for (i = 0; i < nr_atoms; i++) {
		...
		memcpy(&user_atom, user_addr, sizeof(user_atom));
...
#undef compiletime_assert_defined
		...
		sj_dbg_all("SJ: before the statis check! \n");
		...
		while (katom->status != KBASE_JD_ATOM_STATE_UNUSED) {
			...
			sj_dbg_all("SJ: pass kbase_js_sched_all \n");
			...
			if (wait_event_killable(katom->completed,
				 ...
				 * doesn't really matter
				 */
				sj_dbg_all("SJ: ifelse We're being killed \n");

				return 0;
			}

			sj_dbg_all("SJ: pass killable_check \n");
...
KBASE_EXPORT_TEST_API(kbase_jd_submit_SJ_kernel);
