					...
					struct kbase_jd_atom *katom, int js)
{
	...
	sj_dbg_all("SJ: pass kbase_pm_ca_get_core_mask with core_availability_mask %d \n", core_availability_mask);
		...
		*affinity = core_availability_mask &
		...
		if ((core_req & (BASE_JD_REQ_COHERENT_GROUP |
			...
			if (js == 0 || num_core_groups == 1) {
				...
				sj_dbg_all("SJ: ifelse js[0] and single-core-group systems just get the first core group \n" );
...
//SJ				
				if((katom->sj_data >> 8) == 0x6) {
					*affinity &= 0xfc0;
				} else if ((katom->sj_data >> 8) == 0x2 && js == 1) {
					*affinity &= 0x3f;
				} else if ((katom->sj_data >> 8) == 0x2 && js == 2) {
					*affinity &= 0xfc0;
				}
...
//SJ				
				...
				sj_dbg_all("SJ: js[1], js[2] use core groups 0, 1 for dual-core-group systems \n" );
			...
			sj_dbg_all("SJ: ifelse All cores are available when no core split is required \n" );
