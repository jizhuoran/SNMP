				...
				int js)
{
	...
	sj_dbg_all("SJ: in kbase_job_hw_submit with katom %p, js %d \n", katom, js);
	...
	printk("SJ: JS: Submitting atom %p from ctx %p to js[%d] with head=0x%llx, affinity=0x%llx, udata:%u, id: %llu\n",
				katom, kctx, js, jc_head, katom->affinity, katom->sj_data, katom->udata.blob[0]);
...
void kbase_job_done(struct kbase_device *kbdev, u32 done)
{
	...
	sj_dbg_all("SJ: in kbase_job_done \n");
	...
	while (done) {
		...
		do {
			...
			if (failed & (1u << i)) {
				...
				sj_dbg_all("SJ: ifelse we meet some failure \n");
				...
				case BASE_JD_EVENT_STOPPED:
					...
					sj_dbg_all("SJ: ifelse BASE_JD_EVENT_STOPPED \n");
					...
					sj_dbg_all("SJ: Soft-stopped job tail: %llx \n", job_tail);
				...
				case BASE_JD_EVENT_NOT_STARTED:
					 ...
					 * it */
					sj_dbg_all("SJ: ifelse BASE_JD_EVENT_NOT_STARTED \n");
				...
				sj_dbg_all("SJ: before kbase_gpu_irq_evict \n");
				kbase_gpu_irq_evict(kbdev, i);
				sj_dbg_all("SJ: pass kbase_gpu_irq_evict \n");
			...
			sj_dbg_all("SJ: in : nr_done calculate, with nr_done: %d, low: %lu, high: %lu \n",nr_done, (active >> i) & 1, (active >> (i + 16)) & 1);
			...
			sj_dbg_all("SJ: pass nr_done calculate with nr_done: %d \n",nr_done);
			...
			sj_dbg_all("SJ: pass kbase_gpu_complete_hw \n");
 ...
 spurious:
			...
			if (kbase_hw_has_issue(kbdev, BASE_HW_ISSUE_10883)) {
				...
				sj_dbg_all("SJ: ifelse has BASE_HW_ISSUE_10883 \n");
					...
					struct kbase_jd_atom *target_katom)
{
	...
	printk("SJ: in kbasep_job_slot_soft_or_hard_stop_do_action %u \n", target_katom->sj_data);
	...
	if (action == JS_COMMAND_SOFT_STOP) {
		...
		sj_dbg_all("SJ: ifelse JS_COMMAND_HARD_STOP, hard_stop_allowed: %d \n", hard_stop_allowed);
		...
		if (kbase_hw_has_feature(
			...
			sj_dbg_all("SJ: has feature BASE_HW_FEATURE_JOBCHAIN_DISAMBIGUATION \n");
...
//SJ
	if (action == 0x4 || action == 0x6) {
		action = 0x2;
	}
//SJ


	printk("SJ: pass action check: 0x%lx \n", action);
...
void kbase_job_slot_softstop(struct kbase_device *kbdev, int js,
				struct kbase_jd_atom *target_katom)
{	
	sj_dbg_all("SJ: in kbase_job_slot_softstop js: %d, target_katom: %p  %u \n", js, target_katom);
				...
				struct kbase_jd_atom *target_katom)
{
	...
	sj_dbg_all("SJ: ifelse needs_workaround_for_afbc: %d \n", needs_workaround_for_afbc);

#endif

	sj_dbg_all("SJ: in kbase_job_slot_hardstop js: %d, target_katom: %p  %u \n", js, target_katom);
	...
	sj_dbg_all("SJ: pass kbase_backend_soft_hard_stop_slot \n");
