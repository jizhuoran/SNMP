					...
					enum kbasep_pm_action action)
{
	...
	sj_dbg_all("SJ: in kbase_pm_invoke action:%d ,cores:%llu \n", action, cores);
						...
						u64 *powering_on)
{
	...
	sj_dbg_all("SJ: in kbase_pm_transition_core_type, type:%d, desired_state:%llu, in_use:%llu \n", type, desired_state, in_use);
	...
	sj_dbg_all("SJ: pass kbase_pm_is_suspending trans:%llu ,ready:%llu \n", trans, ready);
	...
	*powering_on = powering_on_trans;
	...
	sj_dbg_all("SJ: pass kbase_pm_is_suspending desired_state\n");
		...
		u64 tilers_powered)
{
	...
	sj_dbg_all("SJ: in get_desired_cache_status present:%llu, cores_powered:%llu, tilers_powered:%llu \n", present, cores_powered, tilers_powered);
...
MOCKABLE(kbase_pm_check_transitions_nolock) (struct kbase_device *kbdev)
{
	...
	sj_dbg_all("SJ: in kbase_pm_check_transitions_nolock \n");
	...
	sj_dbg_all("SJ: pass kbase_pm_check_transitions_nolock gpu_powered \n");
	...
	sj_dbg_all("SJ: pass kbase_pm_get_ready_cores %llu \n", cores_powered);

	
	cores_powered |= kbdev->pm.backend.desired_shader_state;

	sj_dbg_all("SJ: pass desired_shader_state %llu \n", kbdev->pm.backend.desired_shader_state);
	...
	sj_dbg_all("SJ: pass get_desired_cache_status %llu \n", desired_l2_state);
	...
	sj_dbg_all("SJ: pass kbase_pm_transition_core_type %d \n", in_desired_state);
	...
	if (in_desired_state) {
		...
		sj_dbg_all("SJ: ifelse in_desired_state \n");
		...
		sj_dbg_all("SJ: pass kbase_pm_check_transitions_nolock shader_available_bitmap %llu \n", shader_available_bitmap);
		...
		sj_dbg_all("SJ: ifelse tiler_present \n");
...
void kbase_pm_clock_on(struct kbase_device *kbdev, bool is_resume)
{
	...
	sj_dbg_all("SJ: in kbase_pm_clock_on withgpu_powered %d \n", kbdev->pm.backend.gpu_powered);
	...
	sj_dbg_all("SJ: pass gpu_powered on with reset_required %d \n", reset_required);
	...
	sj_dbg_all("SJ: pass kbase_pm_init_hw \n");
...
int kbase_pm_init_hw(struct kbase_device *kbdev, unsigned int flags)
{
	...
	sj_dbg_all("SJ: in kbase_pm_init_hw \n");

	/* Ensure the clock is on before attempting to access the hardware */
	if (!kbdev->pm.backend.gpu_powered) {
		if (kbdev->pm.backend.callback_power_on){
			sj_dbg_all("SJ: ifelse we have callback_power_on \n");
			kbdev->pm.backend.callback_power_on(kbdev);
		}
	...
	sj_dbg_all("SJ: goint to Soft reset the GPU wtih protected_support %d \n", kbdev->protected_mode_support);
	...
	sj_dbg_all("SJ: pass Soft reset the GPU \n");
