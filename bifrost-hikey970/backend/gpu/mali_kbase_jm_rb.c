					...
					struct kbase_jd_atom *katom)
{
	...
	sj_dbg_all("SJ: in kbase_gpu_enqueue_atom with katom %p \n", katom);
						...
						struct kbase_jd_atom *katom)
{
	...
	do {
		...
		sj_dbg_all("SJ: in kbasep_js_job_check_ref_cores with katom->coreref_state %u \n", katom->coreref_state);
		...
		case KBASE_ATOM_COREREF_STATE_NO_CORES_REQUESTED:
			...
			if (false == kbase_js_choose_affinity(
				...
				sj_dbg_all("SJ: ifelse No cores are currently available \n");
			...
			sj_dbg_all("SJ: pass KBASE_ATOM_COREREF_STATE_NO_CORES_REQUESTED with affinity %llu \n", recently_chosen_affinity);
		...
		case KBASE_ATOM_COREREF_STATE_WAITING_FOR_REQUESTED_CORES:
			{
				...
				if (cores_ready == KBASE_NEW_AFFINITY) {
					...
					sj_dbg_all("SJ: ifelse KBASE_NEW_AFFINITY \n");
				...
				if (cores_ready == KBASE_CORES_NOT_READY) {
					...
					sj_dbg_all("SJ: ifelse KBASE_CORES_NOT_READY \n");
			...
			sj_dbg_all("SJ: pass KBASE_ATOM_COREREF_STATE_WAITING_FOR_REQUESTED_CORES \n");
		...
		case KBASE_ATOM_COREREF_STATE_RECHECK_AFFINITY:
			...
			sj_dbg_all("SJ: pass KBASE_ATOM_COREREF_STATE_RECHECK_AFFINITY \n");
		...
		case KBASE_ATOM_COREREF_STATE_CHECK_AFFINITY_VIOLATIONS:
			...
			sj_dbg_all("SJ: pass KBASE_ATOM_COREREF_STATE_CHECK_AFFINITY_VIOLATIONS \n");
...
void kbase_backend_slot_update(struct kbase_device *kbdev)
{
	...
	sj_dbg_all("SJ: in kbase_backend_slot_update\n");
	...
	for (js = 0; js < kbdev->gpu_props.num_job_slots; js++) {
		...
		sj_dbg_all("SJ: we have find katom[0]: %p, katom[1]: %p \n", katom[0], katom[1]);
		...
		for (idx = 0; idx < SLOT_RB_SIZE; idx++) {
			...
			sj_dbg_all("SJ: pass find first katom %p in slot %d with idx %d and state %u \n", katom[idx], js, idx, katom[idx]->gpu_rb_state);
			...
			case KBASE_ATOM_GPU_RB_WAITING_BLOCKED:
				...
				sj_dbg_all("SJ: pass KBASE_ATOM_GPU_RB_WAITING_BLOCKED \n");
			...
			case KBASE_ATOM_GPU_RB_WAITING_PROTECTED_MODE_PREV:
				...
				sj_dbg_all("SJ: pass KBASE_ATOM_GPU_RB_WAITING_PROTECTED_MODE_PREV \n");
			...
			case KBASE_ATOM_GPU_RB_WAITING_PROTECTED_MODE_TRANSITION:
				...
				katom[idx]->gpu_rb_state =
					KBASE_ATOM_GPU_RB_WAITING_FOR_CORE_AVAILABLE;
				sj_dbg_all("SJ: pass KBASE_ATOM_GPU_RB_WAITING_PROTECTED_MODE_TRANSITION \n");
			...
			case KBASE_ATOM_GPU_RB_WAITING_FOR_CORE_AVAILABLE:
				if (katom[idx]->will_fail_event_code) {
					...
					sj_dbg_all("SJ: ifelse will_fail_event_code \n");
				...
				sj_dbg_all("SJ: pass kbasep_js_job_check_ref_cores with cores_ready %d \n", cores_ready);

				if (katom[idx]->event_code ==
						BASE_JD_EVENT_PM_EVENT) {

					sj_dbg_all("SJ: ifelse BASE_JD_EVENT_PM_EVENT \n");
				...
				sj_dbg_all("SJ: pass KBASE_ATOM_GPU_RB_WAITING_FOR_CORE_AVAILABLE \n");
			...
			case KBASE_ATOM_GPU_RB_WAITING_AFFINITY:
				...
				sj_dbg_all("SJ: pass KBASE_ATOM_GPU_RB_WAITING_AFFINITY \n");
			...
			case KBASE_ATOM_GPU_RB_READY:
				...
				sj_dbg_all("SJ: pass KBASE_ATOM_GPU_RB_READY \n");

			/* ***FALLTHROUGH: TRANSITION TO HIGHER STATE*** */

			case KBASE_ATOM_GPU_RB_SUBMITTED:
				sj_dbg_all("SJ: pass KBASE_ATOM_GPU_RB_SUBMITTED \n");
				/* Atom submitted to HW, nothing else to do */
				break;

			case KBASE_ATOM_GPU_RB_RETURN_TO_JS:
				sj_dbg_all("SJ: pass KBASE_ATOM_GPU_RB_RETURN_TO_JS \n");
				/* Only return if head atom or previous atom
				 * already removed - as atoms must be returned
				 * in order */
				if (idx == 0 || katom[0]->gpu_rb_state ==
				...
				struct kbase_jd_atom *katom)
{
	...
	sj_dbg_all("SJ: in kbase_backend_run_atom with katom %p \n", katom);
					...
					u32 action)
{
	...
	sj_dbg_all("SJ: in kbase_gpu_stop_atom action: %lu \n", hw_action);

	kbase_job_check_enter_disjoint(kbdev, action, katom->core_req, katom);

	sj_dbg_all("SJ: pass kbase_job_check_enter_disjoint  \n");


	kbasep_job_slot_soft_or_hard_stop_do_action(kbdev, js, hw_action,
							katom->core_req, katom);

	sj_dbg_all("SJ: pass kbasep_job_slot_soft_or_hard_stop_do_action  \n");
					...
					u32 action)
{
	...
	sj_dbg_all("SJ: in kbase_backend_soft_hard_stop_slot katom_idx0: %p, katom_idx1: %p \n", katom_idx0, katom_idx1);
	...
	sj_dbg_all("SJ: pass valid check katom_idx0_valid: %d, katom_idx1_valid: %d \n", katom_idx0_valid, katom_idx1_valid);
	...
	sj_dbg_all("SJ: pass stop dep check stop_x_dep_idx0: %d, stop_x_dep_idx1: %d \n", stop_x_dep_idx0, stop_x_dep_idx1);


	if (katom_idx0_valid) {
		if (katom_idx0->gpu_rb_state != KBASE_ATOM_GPU_RB_SUBMITTED) {

			sj_dbg_all("SJ: ifelse katom_idx0_valid and NOT SUBMITTED \n");
			...
			if (katom_idx1 && katom_idx1->gpu_rb_state ==
				...
				sj_dbg_all("SJ: ifelse katom_idx0_valid, has idx1 and idx1 SUBMITTED \n");
				...
				sj_dbg_all("SJ: ifelse katom_idx0_valid, has idx1 and idx1 NOT SUBMITTED \n");
				...
				sj_dbg_all("SJ: ifelse katom_idx0_valid, NO idx1  \n");
		...
		if (katom_idx1->gpu_rb_state != KBASE_ATOM_GPU_RB_SUBMITTED) {
			...
			sj_dbg_all("SJ: ifelse katom_idx0_NOT_valid, katom_idx1_valid, idx1 NOT SUBMITTED \n");
			...
			sj_dbg_all("SJ: ifelse katom_idx0_NOT_valid, katom_idx1_valid, idx1 SUBMITTED \n");
