...
static inline bool timer_callback_should_run(struct kbase_device *kbdev)
{
	...
	sj_dbg_all("SJ: in timer_callback_should_run %d \n", nr_running_ctxs);
	...
	if (kbase_hw_has_issue(kbdev, BASE_HW_ISSUE_9435)) {
...

		{
			...
			sj_dbg_all("SJ: ifelse has BASE_HW_ISSUE_9435: nr_compute_ctxs:%d and nr_noncompute_ctxs:%d \n", nr_compute_ctxs, nr_noncompute_ctxs);
		...
		sj_dbg_all("SJ: ifelse NOT has BASE_HW_ISSUE_9435 \n");
		...
		if (atom != NULL) {
			...
			if (!kbase_hw_has_issue(kbdev, BASE_HW_ISSUE_5736)) {
				...
				if (ticks == soft_stop_ticks) {
					...
					sj_dbg_all("SJ: notice going to soft-stop \n");
...
void kbase_backend_ctx_count_changed(struct kbase_device *kbdev)
{
	...
	if (!timer_callback_should_run(kbdev)) {
		...
		sj_dbg_all("SJ: ifelse NOT timer_callback_should_run \n");
	...
	if (timer_callback_should_run(kbdev) && !backend->timer_running) {
		...
		sj_dbg_all("SJ: ifelse timer_callback_should_run \n");
