...
void kbase_pm_update_active(struct kbase_device *kbdev)
{
	...
	sj_dbg_all("SJ: in kbase_pm_update_active \n");

	active = backend->pm_current_policy->get_core_active(kbdev);

	sj_dbg_all("SJ: pass get_core_active with active %d \n", active);
	...
	if (active) {
		...
		sj_dbg_all("SJ: pass lots of check of power with poweroff_wait_in_progress %d \n", pm->backend.poweroff_wait_in_progress);
...
void kbase_pm_update_cores_state_nolock(struct kbase_device *kbdev)
{
	...
	sj_dbg_all("SJ: in kbase_pm_update_cores_state_nolock \n");


	sj_dbg_all("SJ: new work, kan zhe ge han shu a!!! \n");
	...
	sj_dbg_all("SJ: pass poweroff_wait_in_progress \n");
	...
	if (kbdev->protected_mode_transition &&	!kbdev->shader_needed_bitmap &&
		...
		sj_dbg_all("SJ: ifelse change in/out of protected mode desired_bitmap:%llu \n", desired_bitmap);

		desired_bitmap &= kbase_pm_ca_get_core_mask(kbdev);

		sj_dbg_all("SJ: pass kbase_pm_ca_get_core_mask desired_bitmap:%llu \n", desired_bitmap);
		...
		if (!kbase_hw_has_feature(kbdev, BASE_HW_FEATURE_XAFFINITY)) {
			...
			if (kbdev->tiler_needed_cnt > 0 ||
				...
				sj_dbg_all("SJ: tiler_needed_cnt > 0 desired_bitmap:%llu \n", desired_bitmap);
	...
	if (~kbdev->pm.backend.desired_shader_state & desired_bitmap ||
		...
		sj_dbg_all("SJ: ifelse any cores being powered on 0 \n");
		...
		sj_dbg_all("SJ DEBUG: the desired_shader_state: 0x%llx, and desired_tiler_bitmap: 0x%llx \n", desired_bitmap, desired_tiler_bitmap);
		...
		sj_dbg_all("SJ: ifelse any cores being powered on 1 \n");
		...
		sj_dbg_all("SJ: ifelse any cores being powered on 2 \n");
	...
	sj_dbg_all("SJ: pass do_poweroff %d \n", do_poweroff);
				...
				bool tiler_required, u64 shader_cores)
{
	...
	if (change_gpu_state) {
		...
		sj_dbg_all("SJ: ifelse change_gpu_state \n");
				...
				bool tiler_required, u64 shader_cores)
{
	...
	sj_dbg_all("SJ: in kbase_pm_register_inuse_cores tiler_required: %d, shader_cores: %llu, shader_available_bitmap %llu \n", tiler_required, shader_cores, kbdev->shader_available_bitmap);
	...
	sj_dbg_all("SJ: pass kbase_pm_register_inuse_cores desired_shader_state with desired_shader_state %llu \n", kbdev->pm.backend.desired_shader_state);
	...
	sj_dbg_all("SJ: pass kbase_pm_register_inuse_cores shader_available_bitmap \n");
