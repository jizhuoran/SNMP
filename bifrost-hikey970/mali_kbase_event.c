...
void kbase_event_post(struct kbase_context *ctx, struct kbase_jd_atom *atom)
{
    ...
    sj_dbg_all("SJ: in kbase_event_post! \n");

	if (atom->core_req & BASE_JD_REQ_EVENT_ONLY_ON_FAILURE) {
		if (atom->event_code == BASE_JD_EVENT_DONE) {
			/* Don't report the event */

    		sj_dbg_all("SJ: ifelse BASE_JD_REQ_EVENT_ONLY_ON_FAILURE and BASE_JD_EVENT_DONE! \n");
	...
	if (atom->core_req & BASEP_JD_REQ_EVENT_NEVER) {
    	...
    	sj_dbg_all("SJ: ifelse BASEP_JD_REQ_EVENT_NEVER! \n");
	...
	if (atom->core_req & BASE_JD_REQ_EVENT_COALESCE) {
    	...
    	sj_dbg_all("SJ: ifelse BASE_JD_REQ_EVENT_COALESCE! \n");
    	...
    	sj_dbg_all("SJ: ifelse Report the event and any pending events now! \n");
