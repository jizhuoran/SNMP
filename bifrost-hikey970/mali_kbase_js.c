...
#include <backend/gpu/mali_kbase_jm_rb.h> //SJ
					...
					bool is_scheduled)
{
	...
	sj_dbg_all("SJ: in kbase_js_ctx_pullable with is_scheduled %d \n", is_scheduled);
	...
	sj_dbg_all("SJ: pass jsctx_rb_peek with katom %p \n", katom);
...
//SJ
	...
	printk("SJ: in submit job type: %u, slot 1: %d, slot 2: %d \n, id: %llu", katom->sj_data, kbase_backend_nr_atoms_on_slot(kctx->kbdev, 1), kbase_backend_nr_atoms_on_slot(kctx->kbdev, 2), katom->udata.blob[0]);

	if((katom->sj_data >> 8) == 0x6) {
		katom->slot_nr = 2;
	} else if((katom->sj_data >> 8) == 0x2){

#ifdef DCAM_ENABLE_TEST_PEAK
		int RT_in_queue = kbase_backend_nr_atoms_on_slot(kctx->kbdev, 1);

		if (RT_in_queue > 0) {
			if (RT_in_queue >= kbase_backend_nr_atoms_on_slot(kctx->kbdev, 2)) {
				katom->slot_nr = 2;
			} else {
				katom->slot_nr = 1;
			}
			...
			for (i = 0; i < kbase_backend_nr_atoms_on_slot(kctx->kbdev, 2); i++) {
				struct kbase_jd_atom *katom_sj;

				katom_sj = kbase_gpu_inspect(kctx->kbdev, 2, i);
				if (!katom_sj)
					continue;

				if ((katom->sj_data >> 8) && !has_preem) {
					kbase_job_slot_hardstop(katom_sj->kctx, 2, katom_sj);
					has_preem = 1;
				}
			}
		...
		if((katom->sj_data) & 0xff == 0x23) {
			katom->slot_nr = 1;
		} else if ((katom->sj_data) & 0xff == 0x21) {
			katom->slot_nr = 2;
		} else {
			katom->slot_nr = 2;
		}
...
void kbase_js_unpull(struct kbase_context *kctx, struct kbase_jd_atom *katom)
{
	...
	sj_dbg_all("SJ: in kbase_js_unpull \n");
...
void kbase_js_sched(struct kbase_device *kbdev, int js_mask)
{
	...
	sj_dbg_all("SJ: in kbase_js_sched \n");
	...
	while (js_mask) {
		...
		sj_dbg_all("SJ: loop js_mask %d in %d \n", js, js_mask);
		...
		while (1) {
			...
			sj_dbg_all("SJ: pass kbase_js_ctx_list_pop_head with kctx %p \n", kctx);
			...
			if (!kbase_ctx_flag(kctx, KCTX_ACTIVE)) {
				...
				if (kbase_pm_context_active_handle_suspend(
					...
					sj_dbg_all("SJ: ifelse kbase_pm_context_active_handle_suspend \n");
			...
			sj_dbg_all("SJ: pass KCTX_ACTIVE \n");


			if (!kbase_js_use_ctx(kbdev, kctx)) {

				sj_dbg_all("SJ: ifelse kbase_js_use_ctx no \n");
			...
			sj_dbg_all("SJ: pass kbase_js_use_ctx \n");
			...
			if (!kbase_jm_kick(kbdev, 1 << js)) {
				/* No more jobs can be submitted on this slot */
				js_mask &= ~(1 << js);
				sj_dbg_all("SJ: No more jobs can be submitted on this slot \n");

			}
				

			sj_dbg_all("SJ: pass kbase_jm_kick \n");
			...
			if (!kbase_ctx_flag(kctx, KCTX_PULLED)) {
				...
				sj_dbg_all("SJ: ifelse kbase_ctx_flag(kctx, KCTX_PULLED) no \n");
			...
			sj_dbg_all("SJ: pass kbase_ctx_flag(kctx, KCTX_PULLED) \n");


			/* Push to back of list */
			if (kbase_js_ctx_pullable(kctx, js, true)){

				sj_dbg_all("SJ: ifelse kbase_js_ctx_pullable yes! \n");
				...
				sj_dbg_all("SJ: ifelse kbase_js_ctx_pullable no! \n");
			...
			sj_dbg_all("SJ: kbase_js_ctx_pullable \n");
	...
	sj_dbg_all("SJ: pass kbase_js_sync_timers with timer_sync: %d \n", timer_sync);
