...
struct kbase_device *kbase_find_device(int minor)
{
	...
	sj_dbg_all("SJ: in kbase_find_device \n");
	...
	list_for_each(entry, dev_list) {
		...
		sj_dbg_all("SJ: the minor is %d", tmp->mdev.minor);
...
static int kbase_open(struct inode *inode, struct file *filp)
{
	...
	sj_dbg_all("SJ: pass created base context \n");
...

	{
		...
		if (element) {
			...
			sj_dbg_all("SJ: couldn't add kctx to kctx_list \n");
...
static int kbase_release(struct inode *inode, struct file *filp)
{
	...
	sj_dbg_all("SJ: in kbase_release \n");


	kbase_destroy_context(kctx);

	sj_dbg_all("SJ: pass kbase_destroy_context \n");

	dev_dbg(kbdev->dev, "deleted base context\n");
	kbase_release_device(kbdev);

	sj_dbg_all("SJ: pass kbase_release_device \n");
...
{
	//((struct base_jd_atom_v2 *)submit->addr)->udata.blob[1]
	



	sj_dbg_all("SJ: the jd submit address is %p \n", u64_to_user_ptr(submit->addr));
...
#ifdef CONFIG_MALI_NO_MALI //SJ
static int kbase_api_kill_job(struct kbase_context *kctx,
		struct kbase_ioctl_job_kill *to_kill)
{

	if(to_kill->hard_stop) {
		kbase_job_slot_hardstop(kctx, to_kill->js, NULL);
	} else {
		kbase_job_slot_softstop(kctx->kbdev, to_kill->js, NULL);
	}
...
static long kbase_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	sj_dbg_all("SJ: ~~~~~~~~in kbase_ioctl~~~~~~~~  %u \n", cmd);
	...
	sj_dbg_all("SJ: before complete check version:%u, setup_complete:%u \n", kctx->api_version, !atomic_read(&kctx->setup_complete));
	...
	sj_dbg_all("SJ: pass complete check \n");
...
#if CONFIG_MALI_NO_MALI //SJ
		KBASE_HANDLE_IOCTL_IN(KBASE_IOCTL_JOB_KILL,
				kbase_api_kill_job,
				struct kbase_ioctl_job_kill);
#endif
...
static int kbase_common_reg_map(struct kbase_device *kbdev)
{
	...
	printk("The start is %llu, the end is %lu and the name is %s \n", kbdev->reg_start, kbdev->reg_size, dev_name(kbdev->dev));
...
static int kbase_platform_device_remove(struct platform_device *pdev)
{
    ...
    sj_dbg_all("SJ: in kbase_platform_device_remove \n ");
    ...
    sj_dbg_all("SJ: pass to_kbase_device\n ");
    ...
    sj_dbg_all("SJ: pass inited_ctx_sched\n ");
    ...
    sj_dbg_all("SJ: pass inited_device\n ");
    ...
    sj_dbg_all("SJ: pass kbase_backend_early_term\n ");
    ...
    sj_dbg_all("SJ: pass inited_io_history\n ");
    ...
    sj_dbg_all("SJ: pass inited_registers_map\n ");


#ifdef CONFIG_MALI_NO_MALI
	if (kbdev->inited_subsys & inited_gpu_device) {

    	sj_dbg_all("SJ: in inited_gpu_device \n ");
		gpu_device_destroy(kbdev);
		kbdev->inited_subsys &= ~inited_gpu_device;
	}
#endif /* CONFIG_MALI_NO_MALI */

    sj_dbg_all("SJ: pass inited_gpu_device\n ");
...
static int kbase_platform_device_probe(struct platform_device *pdev)
{
	...
	sj_dbg_all("SJ:kbase_platform_device_probe!!!\n");
	...
	sj_dbg_all("SJ: the name of mdev is %s!!! \n", kbdev->mdev.name);

	sj_dbg_all("SJ: needtoremove KBASE_IOCTL_VERSION_CHECK: %lu \n", KBASE_IOCTL_VERSION_CHECK);
	sj_dbg_all("SJ: needtoremove KBASE_IOCTL_SET_FLAGS: %lu \n", KBASE_IOCTL_SET_FLAGS);
	sj_dbg_all("SJ: needtoremove KBASE_IOCTL_JOB_SUBMIT: %lu \n", KBASE_IOCTL_JOB_SUBMIT);
	sj_dbg_all("SJ: needtoremove KBASE_IOCTL_GET_GPUPROPS: %lu \n", KBASE_IOCTL_GET_GPUPROPS);
	sj_dbg_all("SJ: needtoremove KBASE_IOCTL_POST_TERM: %lu \n", KBASE_IOCTL_POST_TERM);
	sj_dbg_all("SJ: needtoremove KBASE_IOCTL_MEM_ALLOC: %lu \n", KBASE_IOCTL_MEM_ALLOC);
	sj_dbg_all("SJ: needtoremove KBASE_IOCTL_MEM_QUERY: %lu \n", KBASE_IOCTL_MEM_QUERY);
	sj_dbg_all("SJ: needtoremove KBASE_IOCTL_MEM_FREE: %lu \n", KBASE_IOCTL_MEM_FREE);
	sj_dbg_all("SJ: needtoremove KBASE_IOCTL_HWCNT_READER_SETUP: %lu \n", KBASE_IOCTL_HWCNT_READER_SETUP);
	sj_dbg_all("SJ: needtoremove KBASE_IOCTL_HWCNT_ENABLE: %lu \n", KBASE_IOCTL_HWCNT_ENABLE);
	sj_dbg_all("SJ: needtoremove KBASE_IOCTL_HWCNT_DUMP: %lu \n", KBASE_IOCTL_HWCNT_DUMP);
	sj_dbg_all("SJ: needtoremove KBASE_IOCTL_HWCNT_CLEAR: %lu \n", KBASE_IOCTL_HWCNT_CLEAR);
	sj_dbg_all("SJ: needtoremove KBASE_IOCTL_DISJOINT_QUERY: %lu \n", KBASE_IOCTL_DISJOINT_QUERY);
	sj_dbg_all("SJ: needtoremove KBASE_IOCTL_GET_DDK_VERSION: %lu \n", KBASE_IOCTL_GET_DDK_VERSION);
	sj_dbg_all("SJ: needtoremove KBASE_IOCTL_MEM_JIT_INIT: %lu \n", KBASE_IOCTL_MEM_JIT_INIT);
	sj_dbg_all("SJ: needtoremove KBASE_IOCTL_MEM_SYNC: %lu \n", KBASE_IOCTL_MEM_SYNC);
	sj_dbg_all("SJ: needtoremove KBASE_IOCTL_MEM_FIND_CPU_OFFSET: %lu \n", KBASE_IOCTL_MEM_FIND_CPU_OFFSET);
	sj_dbg_all("SJ: needtoremove KBASE_IOCTL_MEM_FIND_GPU_START_AND_OFFSET: %lu \n", KBASE_IOCTL_MEM_FIND_GPU_START_AND_OFFSET);
	sj_dbg_all("SJ: needtoremove KBASE_IOCTL_GET_CONTEXT_ID: %lu \n", KBASE_IOCTL_GET_CONTEXT_ID);
	sj_dbg_all("SJ: needtoremove KBASE_IOCTL_TLSTREAM_ACQUIRE: %lu \n", KBASE_IOCTL_TLSTREAM_ACQUIRE);
	sj_dbg_all("SJ: needtoremove KBASE_IOCTL_TLSTREAM_FLUSH: %lu \n", KBASE_IOCTL_TLSTREAM_FLUSH);
	sj_dbg_all("SJ: needtoremove KBASE_IOCTL_MEM_COMMIT: %lu \n", KBASE_IOCTL_MEM_COMMIT);
	sj_dbg_all("SJ: needtoremove KBASE_IOCTL_MEM_ALIAS: %lu \n", KBASE_IOCTL_MEM_ALIAS);
	sj_dbg_all("SJ: needtoremove KBASE_IOCTL_MEM_IMPORT: %lu \n", KBASE_IOCTL_MEM_IMPORT);
	sj_dbg_all("SJ: needtoremove KBASE_IOCTL_MEM_FLAGS_CHANGE: %lu \n", KBASE_IOCTL_MEM_FLAGS_CHANGE);
	sj_dbg_all("SJ: needtoremove KBASE_IOCTL_STREAM_CREATE: %lu \n", KBASE_IOCTL_STREAM_CREATE);
	sj_dbg_all("SJ: needtoremove KBASE_IOCTL_FENCE_VALIDATE: %lu \n", KBASE_IOCTL_FENCE_VALIDATE);
	sj_dbg_all("SJ: needtoremove KBASE_IOCTL_GET_PROFILING_CONTROLS: %lu \n", KBASE_IOCTL_GET_PROFILING_CONTROLS);
	sj_dbg_all("SJ: needtoremove KBASE_IOCTL_MEM_PROFILE_ADD: %lu \n", KBASE_IOCTL_MEM_PROFILE_ADD);
	sj_dbg_all("SJ: needtoremove KBASE_IOCTL_SOFT_EVENT_UPDATE: %lu \n", KBASE_IOCTL_SOFT_EVENT_UPDATE);
...
static int __init kbase_driver_init(void)
{
	...
	sj_dbg_all("SJ: kbase_driver_init!!!\n");
