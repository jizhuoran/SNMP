		...
		u64 *gpu_va)
{
	...
	sj_dbg_all("SJ: in kbase_mem_alloc: va_pages: %llu, commit_pages: %llu, extent: %llu \n", va_pages, commit_pages, extent);
	...
	*gpu_va = 0; /* return 0 on failure */
	...
	sj_dbg_all("SJ: pass kbase_alloc_free_region, reg_addr: %p, nr_pages: %lu \n", reg, reg->nr_pages);
	...
	if (*flags & BASE_MEM_SAME_VA) {
		...
		sj_dbg_all("SJ: pass kbase_mem_alloc cookies %p, cookie_nr: %lu with api_version %lu \n", cookie, cookie_nr, kctx->api_version);
		...
		if (kctx->api_version < KBASE_API_VERSION(10, 1) ||
			...
			*gpu_va = (u64) cookie;
			...
			sj_dbg_all("SJ: ifelse return cookie reg_addr: %p, flags: %llu \n", reg, reg->flags);

			return reg;
		}

		sj_dbg_all("SJ: pass relies on the kernel to call mmap \n");
		...
		sj_dbg_all("SJ: pass vm_mmap cpu_addr: %llu \n", cpu_addr);
...

{
	...
	*nr_pages = kbase_reg_current_backed_size(reg);
    ...
    sj_dbg_all("SJ: pass kbase_gpu_mmap \n");
...
int kbase_mmap(struct file *file, struct vm_area_struct *vma)
{
    ...
    sj_dbg_all("SJ: in kbase_mmap kctx: %p, start:%p, end:%p, page_num: %lu \n", kctx, vma->vm_start, vma->vm_end, nr_pages);
    ...
    sj_dbg_all("SJ: pass VM_SHARED check \n");
    ...
    sj_dbg_all("SJ: pass BASE_MEM_MAP_TRACKING_HANDLE check \n");
	...
	if (rcu_dereference(kctx->process_mm) != current->mm) {
    	...
    	sj_dbg_all("SJ: ifelse fail, rcu_dereference: %p, current->mm: %p \n", rcu_dereference(kctx->process_mm), current->mm);
    ...
    sj_dbg_all("SJ: pass kctx->process_mm check \n");


	switch (vma->vm_pgoff) {
	case PFN_DOWN(BASEP_MEM_INVALID_HANDLE):
    	sj_dbg_all("SJ: BASEP_MEM_INVALID_HANDLE \n");
	case PFN_DOWN(BASEP_MEM_WRITE_ALLOC_PAGES_HANDLE):
    	sj_dbg_all("SJ: BASEP_MEM_WRITE_ALLOC_PAGES_HANDLE \n");
		/* Illegal handle for direct map */
    	sj_dbg_all("SJ: ifelse Illegal handle for direct map \n");
		err = -EINVAL;
		goto out_unlock;
	case PFN_DOWN(BASE_MEM_TRACE_BUFFER_HANDLE):
    	sj_dbg_all("SJ: ifelse BASE_MEM_TRACE_BUFFER_HANDLE\n");
	...
	case PFN_DOWN(BASE_MEM_MMU_DUMP_HANDLE):
		/* MMU dump */
    	sj_dbg_all("SJ: ifelse BASE_MEM_MMU_DUMP_HANDLE\n");
	...
	case PFN_DOWN(BASE_MEM_COOKIE_BASE) ...
    	...
    	sj_dbg_all("SJ: ifelse BASE_MEM_COOKIE_BASE \n");

		err = kbasep_reg_mmap(kctx, vma, &reg, &nr_pages,
							&aligned_offset);

    	sj_dbg_all("SJ: pass kbasep_reg_mmap \n");
    ...
    sj_dbg_all("SJ: pass kbase_cpu_mmap \n");
    ...
    sj_dbg_all("SJ: pass MMU dump check \n");
...
out:
    ...
    sj_dbg_all("SJ: pass final check \n");
