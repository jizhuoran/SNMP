...
struct page *kbase_mem_alloc_page(struct kbase_mem_pool *pool)
{
	...
	// sj_dbg_all("SJ: in kbase_mem_alloc_page\n");
	...
	// sj_dbg_all("SJ: alloc_pages dev: %p page: %p pool->order:%llu \n", dev, p, (PAGE_SIZE << pool->order));
	...
	// sj_dbg_all("SJ: dma_map_page with phys_addr: %p, dma_addr: %p \n", page_to_phys(p), dma_addr);
		...
		size_t nr_to_grow)
{
	...
	sj_dbg_all("SJ: in kbase_mem_pool_grow\n");
	...
	for (i = 0; i < nr_to_grow; i++) {
		...
		sj_dbg_all("SJ: kbase_mem_alloc_page\n");
		...
		struct tagged_addr *pages, bool partial_allowed)
{
	...
	sj_dbg_all("SJ: in kbase_mem_pool_alloc_pages nr_4k_pages: %lu, nr_pages_internal: %lu \n", nr_4k_pages, nr_pages_internal);
	...
	sj_dbg_all("SJ: pass nr_from_pool: %lu \n", nr_from_pool);
	...
	sj_dbg_all("SJ: ifelse i: %lu, i != nr_4k_pages: %d, pool->next_pool: %p \n", i, i != nr_4k_pages, pool->next_pool);
