						    ...
						    struct kbase_va_region *reg)
{
	...
	case KBASE_REG_ZONE_CUSTOM_VA:
		// sj_dbg_all("SJ: ifelsekbase_reg_flags_to_rbtree KBASE_REG_ZONE_CUSTOM_VA \n");
		rbtree = &kctx->reg_rbtree_custom;
		break;
	case KBASE_REG_ZONE_EXEC:
		// sj_dbg_all("SJ: ifelsekbase_reg_flags_to_rbtree KBASE_REG_ZONE_EXEC \n");
		rbtree = &kctx->reg_rbtree_exec;
		break;
	case KBASE_REG_ZONE_SAME_VA:
		// sj_dbg_all("SJ: ifelsekbase_reg_flags_to_rbtree KBASE_REG_ZONE_SAME_VA \n");
								    ...
								    u64 gpu_pfn)
{
	...
	if (kbase_ctx_flag(kctx, KCTX_COMPAT)) {
		...
		if (gpu_pfn >= KBASE_REG_ZONE_CUSTOM_VA_BASE){
			...
			sj_dbg_all("SJ: ifelse KBASE_REG_ZONE_CUSTOM_VA_BASE \n");

			rbtree = &kctx->reg_rbtree_custom;
		}
		else if (gpu_pfn >= KBASE_REG_ZONE_EXEC_BASE){

			sj_dbg_all("SJ: ifelse KBASE_REG_ZONE_EXEC_BASE \n");
			...
			sj_dbg_all("SJ: ifelse KBASE_REG_ZONE_SAME_VA \n");
...
struct kbase_va_region *kbase_region_tracker_find_region_enclosing_address(struct kbase_context *kctx, u64 gpu_addr)
{
	...
	while (rbnode) {
		...
		sj_dbg_all("SJ: loop the reg address: %p (%llu, %llu) to find %llu \n", reg, tmp_start_pfn, tmp_end_pfn, gpu_pfn);
		...
		u64 *out_start_pfn)
{
	...
	sj_dbg_all("SJ: in kbase_region_tracker_find_region_meeting_reqs %p in %p \n", rbnode, rbtree);


	while (rbnode) {
		reg = rb_entry(rbnode, struct kbase_va_region, rblink);

		sj_dbg_all("SJ: reg->nr_pages: %lu, nr_pages: %lu, reg->flags: %lu \n", reg->nr_pages, nr_pages, reg->flags);
		...
		size_t nr_pages, size_t align)
{
	...
	sj_dbg_all("SJ: in kbase_add_va_region with gpu_pfn %llu, add %llu \n", gpu_pfn, addr);
	...
	sj_dbg_all("SJ: ifelse Map any free address which meets the requirements \n");
	...
	do {
		...
		sj_dbg_all("SJ: pass KBASE_REG_TILER_ALIGN_TOP with err %d \n", err);
		...
		sj_dbg_all("SJ: pass kbase_region_tracker_find_region_meeting_reqs with tmp %d \n", tmp);
...
#error Unsupported 64-bit architecture
...
#ifdef CONFIG_64BIT
    sj_dbg_all("SJ: ifelse define CONFIG_64BIT \n");
#endif
    ...
    sj_dbg_all("SJ: ~~~ in kbase_region_tracker_init, same_va_reg: %p, exec_reg: %p, custom_va_reg: %p \n", same_va_reg, exec_reg, custom_va_reg);
...
int kbase_gpu_mmap(struct kbase_context *kctx, struct kbase_va_region *reg, u64 addr, size_t nr_pages, size_t align)
{
	...
	sj_dbg_all("SJ: in kbase_gpu_mmap \n");
	...
	sj_dbg_all("SJ: pass kbase_add_va_region \n");


	if (reg->gpu_alloc->type == KBASE_MEM_TYPE_ALIAS) {

		sj_dbg_all("SJ: ifelse reg->gpu_alloc->type == KBASE_MEM_TYPE_ALIAS \n");
		...
		sj_dbg_all("SJ: ifelse NOT KBASE_MEM_TYPE_ALIAS \n");
		...
		sj_dbg_all("SJ: pass kbase_mmu_insert_pages \n");


		kbase_mem_phy_alloc_gpu_mapped(reg->gpu_alloc);
	}

	sj_dbg_all("SJ: pass kbase_mem_phy_alloc_gpu_mapped \n");
		...
		struct basep_syncset *sset, enum kbase_sync_type sync_fn)
{
	...
	sj_dbg_all("SJ: in kbase_do_syncset \n");
	...
	sj_dbg_all("SJ: pass kbase_region_tracker_find_region_enclosing_address handle: %p, reg: %p, reg.flags: %p \n", sset->mem_handle.basep.handle, reg, reg->flags);


	if (!(reg->flags & KBASE_REG_CPU_CACHED) ||
			kbase_mem_is_imported(reg->gpu_alloc->type)) {
		sj_dbg_all("SJ: ifelse kbase_mem_is_imported, CPU_CACHED: %d, this will fail \n", (reg->flags & KBASE_REG_CPU_CACHED));
		goto out_unlock;
	...
	sj_dbg_all("SJ: pass kbasep_find_enclosing_cpu_mapping \n");
...
int kbase_mem_free(struct kbase_context *kctx, u64 gpu_addr)
{
	...
	sj_dbg_all("SJ: in kbase_mem_free \n");
	...
	sj_dbg_all("SJ: pass parameter check \n");
	...
	sj_dbg_all("SJ: pass reserved for the ringbuffer check \n");
	...
	if (gpu_addr >= BASE_MEM_COOKIE_BASE &&
		...
		sj_dbg_all("SJ: ifelse to free a cookie %p \n", reg);
		...
		sj_dbg_all("SJ: pass kbase_free_alloced_region \n");

	} else {

		sj_dbg_all("SJ: ifelse A real GPU va \n");
		...
		sj_dbg_all("SJ: pass kbase_region_tracker_find_region_base_address %p \n", reg);
		...
		sj_dbg_all("SJ: pass KBASE_REG_ZONE_SAME_VA %p \n", reg);
	...
	size_t nr_pages_requested)
{
	...
	sj_dbg_all("SJ: in kbase_alloc_phy_pages_helper nr_pages_requested %lu, alloc->reg: %p \n", nr_pages_requested, alloc->reg);
	...
	sj_dbg_all("SJ: ifelse CONFIG_MALI_2MB_ALLOC \n", CONFIG_MALI_2MB_ALLOC);
...
no_new_partial:
	...
	sj_dbg_all("SJ: pass kbase_mem_pool_alloc_pages \n");
...
int kbase_alloc_phy_pages(struct kbase_va_region *reg, size_t vsize, size_t size)
{
	...
	sj_dbg_all("SJ: in kbase_alloc_phy_pages vsize: %lu, size: %lu \n", vsize, size);
	...
	sj_dbg_all("SJ: ifelse reg->cpu_alloc == reg->gpu_alloc: %d, reg: %p \n", reg->cpu_alloc == reg->gpu_alloc, reg);
